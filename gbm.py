...
def generate_brownian_motion(total_time, sim_period, mean, stdev):
    ...
    normal_values = np.random.standard_normal(size=n)  # normal distribution
    W = np.cumsum(normal_values) * np.sqrt(sim_period)  # brownian motion (Weiner process)
...
def multi_generate_brownian_motion(total_time, sim_period, mean, stdev, corr_matrix):
    """multivariate version like above with a correlation matrix
    Based on https://gist.github.com/athirara/49bf24cbffdc0a07343fad09ccd7f411 (and corrected errors..)
    """
    ...
    normal_values = np.random.standard_normal(size=(n, num_assets))  # normal distribution
